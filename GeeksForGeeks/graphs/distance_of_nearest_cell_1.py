...
class Solution:
    ...
    def nearest(self, grid) -> List[List[int]]:
        ...
        while len(q) > 0: 
            i, j, cur_dis = q.popleft()
            for mod_i, mod_j in possible_coords:
                new_i = i + mod_i
                new_j = j + mod_j
                if new_i >= 0 and new_i < n and new_j >= 0 and new_j < m and visited[new_i][new_j] == 0 and grid[new_i][new_j] == 0:
                    q.append((new_i, new_j, cur_dis + 1))
                    visited[new_i][new_j] = 1
                    grid[new_i][new_j] = cur_dis + 1
