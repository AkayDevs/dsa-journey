        ...
        for i in range(n):
            for j in range(m):
                if visited[i][j] == 0 and grid[i][j] == 1:
                    curr_order = self.traverse(grid, visited, (i, j))
                    traversel_list.append(curr_order)

        return len({tuple(sublist) for sublist in traversel_list})


    def traverse(self, grid, visited, src) -> List[tuple[int]]:
        q = deque()
        traverse_order = []
        n = len(grid)
        m = len(grid[0])
        ...
        q.append(src)
        traverse_order.append(src)

        delta_coords = [(0, 1), (0, -1), (1, 0), (-1, 0)]
        while len(q) > 0:
            i, j = q.popleft()
            for delta_i, delta_j in delta_coords:
                new_i = delta_i + i
                new_j = delta_j + j
                if new_i >= 0 and new_i < n and new_j >= 0 and new_j < m and visited[new_i][new_j] == 0 and grid[new_i][new_j] == 1:
                    q.append((new_i, new_j))
                    visited[new_i][new_j] = 1
                    traverse_order.append((new_i, new_j))

        traverse_order.sort()
        base_elem = traverse_order[0]

        return [ (i - base_elem[0], j - base_elem[1]) for i, j in traverse_order]
...
def main():
    ...
    try:
        ...
        n = read_int()
        m = read_int()
        grid = [ read_list_int() for _ in range(n)]
