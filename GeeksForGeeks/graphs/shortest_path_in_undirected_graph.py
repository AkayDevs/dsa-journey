...
class Solution:
    ...
    """
    def shortestPath(self, adj_list, src) -> List[int]:
        ...
        while len(q) > 0:
            ...
            for neighbour in adj_list[cur_node]:
                if distance_array[neighbour] > cur_dist + 1:
                    distance_array[neighbour] = cur_dist + 1
...
def main():
    ...
    try:
        ...
        # TODO: Parse input
        v = read_int()
        adj_list = [read_list_int() for _ in range(v)]
        src = read_int()
        
        # Create solution object and call the function
        solution = Solution()
        result = solution.shortestPath(adj_list, src)
