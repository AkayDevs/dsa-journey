...
class Solution:
    ...
    def longestPalindromNaive(self, s: str) -> str:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                substr = s[i:j + 1]
                if self.isPalindrome(substr) and len(longest_palindrome) < len(substr):
    ...
    def longestPalindrome(self, s: str) -> str:
        ...
        for i in range(n):
            lptr, rptr = i, i 
            while lptr >= 0 and rptr < n and s[lptr] == s[rptr]:
                if len(resString) < rptr - lptr + 1:
                    resString = s[lptr: rptr + 1]
